...
def letter_seg(lines_img, x_lines, i):
	...
	im2, contours, hierarchy = cv2.findContours(copy_img,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)	
	for cnt in contours:
	...
	for e in range(len(letter)):
		if(letter[e][0]<x_linescopy[0]):
			...
			letter_img = cv2.resize(letter_img_tmp, dsize =(128, 128), interpolation = cv2.INTER_AREA)
			cv2.imwrite('.\seg_img\img'+str(i+1)+'_'+str(word)+'_'+str(letter_index)+'.jpeg', 255-letter_img)
		else:
			x_linescopy.pop(0)
			word += 1
			letter_index = 1
...
def segment(ipimg):
        ...
        im2, contours, hierarchy = cv2.findContours(contr_retrival,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
